...
class Matrix:
	...
	def scalarMultiply(self, scalar):
		newMat = [[v * scalar for v in row] for row in self.getRawDataRef()]
		return Matrix(self.numRows, self.numColumns, newMat)
	
	def __mul__(self, rightMatrix):
		if self.numColumns != rightMatrix.numRows:
			raise NameError("Matrix sizes aren't compatible for multiplication.")
		tRight = rightMatrix.getTranspose()
		result = [[ sum( x*y for (x,y) in zip(self.matrix[j], tRight.matrix[i])) \
	...
	def __add__(self, rightMtrx):
		if self.numColumns != rightMtrx.numColumns or self.numRows != rightMtrx.numRows:
			raise NameError("Matrix sizes aren't compatible for addition.")
		m1 = self.getRawDataRef()
		m2 = rightMtrx.getRawDataRef()
		result = [[a + b for a, b in zip(row1, row2)] for row1, row2 in zip(m1, m2)]
		
		return Matrix(self.numRows, self.numColumns, result)
	
	def __sub__(self, rightMtrx):
		if self.numColumns != rightMtrx.numColumns or self.numRows != rightMtrx.numRows:
			raise NameError("Matrix sizes aren't compatible for addition.")
		m1 = self.getRawDataRef()
		m2 = rightMtrx.getRawDataRef()
	...
	def clone(self):
		clone = [[v for v in row] for row in self.getRawDataRef()]
		return Matrix(self.numRows, self.numColumns, clone)
